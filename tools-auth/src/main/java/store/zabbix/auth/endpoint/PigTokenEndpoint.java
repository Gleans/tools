...
package store.zabbix.auth.endpoint;
...
import store.zabbix.common.security.service.PigUser;
...
public class PigTokenEndpoint {
	 ...
	 */
	@GetMapping("/logout")
	public ResultBean<Boolean> logout(@RequestHeader(value = HttpHeaders.AUTHORIZATION, required = false) String authHeader) {
		if (StringUtils.hasText(authHeader)) {
			String tokenValue = authHeader.replace(OAuth2AccessToken.BEARER_TYPE, StrUtil.EMPTY).trim();
			OAuth2AccessToken accessToken = tokenStore.readAccessToken(tokenValue);
			if (accessToken == null || StrUtil.isBlank(accessToken.getValue())) {
				return new ResultBean<>(false, "退出失败，token 为空");
			}
	 ...
	 */
	@DeleteMapping("/{token}")
	public ResultBean<Boolean> removeToken(@PathVariable("token") String token, @RequestHeader(required = false) String from) {
		if (StrUtil.isBlank(from)) {
			return null;
		}
		return new ResultBean<>(redisTemplate.delete(PROJECT_OAUTH_ACCESS + token));
	}
	 ...
	 */
	@PostMapping("/page")
	public ResultBean getTokenPage(@RequestBody Map<String, Object> params, @RequestHeader(required = false) String from) {
		if (StrUtil.isBlank(from)) {
			return null;
		}
